...
contract EasyAuction {
    ...
    function cancelBuyOrders(
        ...
        for (uint256 i = 0; i < _buyOrders.length; i++) {
            ...
            ) = _buyOrders[i].decodeOrder();
            require(
                userIdOfIter == userId,
                "Only the user can cancel his orders"
            );
            if (buyOrders[auctionId].remove(_buyOrders[i], _prevBuyOrders[i])) {
                claimableAmount = claimableAmount.add(buyAmountOfIter);
                emit CancelationBuyOrders(
                    auctionId,
                    userId,
                    buyAmountOfIter,
                    sellAmountOfIter
        ...
        returns (uint96 priceNumerator, uint96 priceDenominator)
    {
        ...
        if (buyOrders[auctionId].size > 0) {
            ...
            if (buyAmountExceedsSellAmount) {
                ...
                (, priceNumerator, priceDenominator) = clearingPriceOrder
                    .decodeOrder();
        ...
        if (!buyAmountExceedsSellAmount) {
            bytes32 clearingPriceOrder = auctionData[auctionId]
                .initialAuctionOrder;
            auctionData[auctionId].volumeClearingPriceOrder = uint96(
                sumBuyAmount
            );
