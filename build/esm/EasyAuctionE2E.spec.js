...
contract("IterableOrderedOrderSet", async (accounts) => {
    ...
    it("e2e - places a lot of buyOrders and calculates the price to test gas usage of claculatePrice", async () => {
        ...
        const auctionId = await sendTxAndGetReturnValue(easyAuction.initiateAuction, buyToken.address, sellToken.address, 60 * 60, new BN(10).pow(new BN(18)), new BN(10).pow(new BN(18)));
        const nrTests = 5;
        for (let i = 1; i < nrTests; i++) {
            const buyOrder = encodeOrder(1, new BN(i).mul(new BN(10).pow(new BN(18))), new BN(10).pow(new BN(18)).div(new BN(nrTests - 2)));
            let prevBuyOrder = encodeOrder(1, new BN(i - 1).mul(new BN(10).pow(new BN(18))), new BN(10).pow(new BN(18)).div(new BN(nrTests - 2)));
            if (i == 1) {
                prevBuyOrder = queueStartElement;
            }
            await easyAuction.placeBuyOrders(auctionId, [new BN(i).mul(new BN(10).pow(new BN(18)))], [new BN(10).pow(new BN(18)).div(new BN(nrTests - 2))], [prevBuyOrder]);
        }
