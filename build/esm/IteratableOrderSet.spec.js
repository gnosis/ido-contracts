...
contract("IterableOrderedOrderSet", function () {
    ...
    it("should contain the added values", async () => {
        ...
        assert.equal(await set.contains(BYTES32_ONE), false, "The element should not be there");
        ...
        assert.equal(await set.contains(BYTES32_ONE), true, "The element should be there");
        assert.deepEqual(await getSetContent(set), [BYTES32_ONE]);
    });
    it("should insert the same value only once", async () => {
        const set = await IterableOrderedOrderSetWrapper.new();
        assert.equal(await set.insert.call(BYTES32_ONE), true, "First insert should insert");
        await set.insert(BYTES32_ONE);
        assert.deepEqual(await getSetContent(set), [BYTES32_ONE]);
        assert.equal(await set.insert.call(BYTES32_ONE), false, "Second insert should do nothing");
        await set.insert(BYTES32_ONE);
        assert.deepEqual(await getSetContent(set), [BYTES32_ONE]);
    });
    ...
    it("should not allow to insert same limit price with same user", async () => {
        const set = await IterableOrderedOrderSetWrapper.new();
        await set.insert(BYTES32_ONE);
        await truffleAssert.reverts(set.insert(BYTES32_ONE_DIFFERENT), "user is not allowed to place same order twice");
    });
    ...
    it("doesn't allow to insert a number with denominator == 0", async () => {
        const set = await IterableOrderedOrderSetWrapper.new();
        await truffleAssert.reverts(set.insert(BYTES32_ZERO), "Inserting uint96(0) is not supported");
    });
    it("cannot get first of empty list", async () => {
        const set = await IterableOrderedOrderSetWrapper.new();
        await truffleAssert.reverts(set.first(), "Trying to get first from empty set");
    });
    ...
    it("encodeOrder reverses decodeOrder", async () => {
        ...
        assert.equal(await set.encodeOrder.call(ans[0], ans[1], ans[2]), BYTES32_ONE);
    });
});
