...
describe("EasyAuction", async () => {
  ...
  it("e2e - to test the gas consumed by precalculateSellAmountSum", async () => {
    const {
      auctioningToken,
      biddingToken,
    } = await createTokensAndMintAndApprove(easyAuction, [user_1, user_2], hre);
    const nrTests = 100; // increase here for better gas estimations, nrTests-2 must be a divisor of 10**18
    const auctionId: BigNumber = await sendTxAndGetReturnValue(
      easyAuction,
      "initiateAuction(address,address,uint256,uint256,uint96,uint96,uint256,uint256,bool)",
      auctioningToken.address,
    ...
    await closeAuction(easyAuction, auctionId);
    const tx = await easyAuction.precalculateSellAmountSum(
      auctionId,
      nrTests - 3,
    );
    const gasUsed = (await tx.wait()).gasUsed;
