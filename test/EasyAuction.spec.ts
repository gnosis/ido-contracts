...
function toAuctionDataResult(
  result: [string, string, BN, string, string, BN]
): OrderResult {
  return {
    sellToken: result[0],
    buyToken: result[1],
  ...
  describe("placeOrders", async () => {
    ...
    it("places a new order and checks that tokens were transferred", async () => {
      ...
      await easyAuction.placeBuyOrders(
        auctionId,
        [buyAmount, buyAmount],
        [sellAmount, sellAmount.add(new BN(1))],
        [queueStartElement, queueStartElement]
      );
      assert.equal(
        (await buyToken.balanceOf(easyAuction.address)).toString(),
        buyAmount.add(buyAmount).toString()
      );
      assert.equal(
        (await buyToken.balanceOf(user_1)).toString(),
        balanceBeforeOrderPlacement.sub(buyAmount).sub(buyAmount).toString()
      );
    });
